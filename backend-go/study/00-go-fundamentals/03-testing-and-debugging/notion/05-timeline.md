...
```bash
mkdir -p 00-go-fundamentals/03-testing-and-debugging/{go/cmd/debugdemo,go/analyzer,docs/concepts,docs/references,problem}
```
...
### 2-1. Event struct와 ParseLine 함수 (`go/analyzer/analyzer.go`)
...
### 5-1. main.go 작성 (`go/cmd/debugdemo/main.go`)

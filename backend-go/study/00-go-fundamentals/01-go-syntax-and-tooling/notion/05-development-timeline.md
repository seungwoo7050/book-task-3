...
이 프로젝트는 study 디렉터리 아래에 위치한다. Go 소스는 `solution/go/` 하위에, 과제 명세는 `problem/`에, 학습 문서는 `docs/`에 분리했다.
...
### 2-1. lesson 패키지 작성 (`solution/go/lesson/lesson.go`)
...
### 3-1. main.go 작성 (`solution/go/cmd/toolingdemo/main.go`)

```bash
# 파일 생성
touch solution/go/cmd/toolingdemo/main.go
```
...
### 4-1. 테스트 파일 작성 (`solution/go/lesson/lesson_test.go`)
...
| `solution/README.md` | 구현 모듈의 빌드/테스트/상태 요약 |

...
func (s *ProductStore) Create(p *Product) *Product {
	...
	p := s.Create(&Product{
        ...
        - 다음: 마지막엔 현재 저장소에서 다시 돌린 CLI와 남은 질문으로 닫는다.
                ...
                ```bash
(cd /Users/woopinbell/work/book-task-3/study/02-distributed-systems/12-grpc-microservices && cd solution/go && go test -run TestCreateAndGet -v ./server/store)
```
...
```
        ## CLI 2. 현재 저장소에서 다시 돌린 검증
                ...
                ```bash
(cd /Users/woopinbell/work/book-task-3/study/02-distributed-systems/12-grpc-microservices && cd solution/go && go test -v -race -count=1 ./...)
```

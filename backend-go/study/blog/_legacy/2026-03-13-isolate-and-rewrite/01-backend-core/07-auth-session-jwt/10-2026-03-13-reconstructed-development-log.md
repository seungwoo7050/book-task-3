...
```bash
(cd /Users/woopinbell/work/book-task-3/backend-go/study/01-backend-core/07-auth-session-jwt && cd solution/go && go test -v ./internal/auth)
```
...
```bash
(cd /Users/woopinbell/work/book-task-3/backend-go/study/01-backend-core/07-auth-session-jwt && cd solution/go && go test -run TestExpiredToken -v ./internal/auth)
```

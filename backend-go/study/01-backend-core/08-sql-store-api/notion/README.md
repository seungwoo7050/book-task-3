# 08 — SQL Store API notion/

이 폴더는 `08-sql-store-api` 프로젝트의 개발 과정과 학습 기록을 담은 블로그형 에세이 모음이다.
...
- **이전**: 07-auth-session-jwt — 인증/인가 패턴 학습
- **현재**: 08-sql-store-api — `database/sql` 기반 CRUD, migration, optimistic update, transaction rollback
- **다음**: 09-cache-migrations-observability — 캐시, 마이그레이션 도구, 관측성

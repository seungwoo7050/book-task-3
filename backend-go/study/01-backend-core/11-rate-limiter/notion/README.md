# 11 — Rate Limiter notion/

이 폴더는 `11-rate-limiter` 프로젝트의 개발 과정과 학습 기록을 담은 블로그형 에세이 모음이다.
...
- **이전**: 10-concurrency-patterns — Worker Pool, Pipeline, FanOut
- **현재**: 11-rate-limiter — Token Bucket 알고리즘, per-client 제한, HTTP 미들웨어
- **다음**: 12-grpc-microservices — gRPC 기반 마이크로서비스

...
```bash
mkdir -p 01-backend-core/05-http-rest-basics/{go/cmd/server,go/internal/api,docs/concepts,docs/references,problem}
```
...
### 2-1. Task struct 정의 (`go/internal/api/api.go`)
...
### 4-1. main.go 작성 (`go/cmd/server/main.go`)
...
### 5-1. httptest 기반 테스트 (`go/internal/api/api_test.go`)

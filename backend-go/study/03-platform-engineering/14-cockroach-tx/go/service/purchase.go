// Package service implements the business logic for the inventory service.
package service
...
// PurchaseRequest holds the input for a purchase.
...
// PurchaseResponse is the result of a successful purchase.
...
// PurchaseService orchestrates item purchases.
type PurchaseService struct {
	DB         *sql.DB
	MaxRetries int
}

// NewPurchaseService creates a PurchaseService with defaults.
...
// Purchase executes an idempotent purchase within a retryable transaction.
func (s *PurchaseService) Purchase(ctx context.Context, req PurchaseRequest) (*PurchaseResponse, error) {
	...
	err := txn.RunInTx(ctx, s.DB, s.MaxRetries, func(tx *sql.Tx) error {
		...
		// 2. Get player and check balance.
		...
		// 6. Build response and store idempotency key.

...
| :--- | :--- |
| 상태 | `verified` |
| 문제 배경 | `BOJ 1197 최소 스패닝 트리` 문제를 현재 학습 구조에서 설명 가능한 풀이 프로젝트로 다시 정리한 항목 |
| 정식 검증 | `make -C study/Core-0D-MST-Topo/1197/problem test` |
...
## 이 레포의 답
- 한 줄 답: `간선 가중치 정렬 + 유니온파인드로 MST를 구성하는 Kruskal`
...
## 어떻게 검증하나
- 정식 검증: `make -C study/Core-0D-MST-Topo/1197/problem test`
- 대표 실행: `make -C study/Core-0D-MST-Topo/1197/problem run-py`
- 비교 실행: `make -C study/Core-0D-MST-Topo/1197/problem run-cpp`

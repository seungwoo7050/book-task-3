...
- **BOJ 1927** (Core-0A): 최소 힙 — 동일 구조, 부호 반전 불필요
- **BOJ 1715** (Core-0A): 카드 정렬 — 힙 응용
- **BOJ 11286**: 절댓값 힙 — `(|x|, x)` 튜플 힙

## 다시 연결해 볼 질문

- `최대 힙`에서 쓴 개념이 같은 트랙의 다음 문제에서 어떻게 더 어려워지는가?
...
- `최대 힙`를 다시 설명할 때는 문제 이름보다 `우선순위 큐가 필요한 상황을 식별하고 비교 기준을 일관되게 유지하는 연습`를 먼저 말한다.

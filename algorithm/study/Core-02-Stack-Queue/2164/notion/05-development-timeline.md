...
- 현재 기준 경로: `study/Core-02-Stack-Queue/2164`
...
provenance 메모: 이전 마이그레이션 기록상 원본 경로는 `legacy/core/02-stack-queue/silver-2164`였다. 현재 읽을 기준 경로는 `study/Core-02-Stack-Queue/2164`이다.
...
- `카드2`에서는 `명령 기반 자료구조 문제를 상태 전이 규칙으로 정리하는 연습`를 실제 명령과 문서 흐름으로 다시 밟아 볼 수 있어야 학습 기록으로서 가치가 생긴다.

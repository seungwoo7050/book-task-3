...
- `python/src/solution.py`에서 `커서를 기준으로 좌/우 스택(또는 리스트) 두 개를 유지하는 editor simulation`를 실제 상태 전이로 옮긴다.
...
- 당시 목표: `커서를 기준으로 좌/우 스택(또는 리스트) 두 개를 유지하는 editor simulation`를 Python 한 파일에서 바로 읽히는 상태 전이로 만든다.
...
문제를 단순하게 만든 건 화려한 알고리즘 이름보다 setup였다. 어떤 값을 오래 들고 갈지 정하는 순간 `커서를 기준으로 좌/우 스택(또는 리스트) 두 개를 유지하는 editor simulation`의 뼈대가 이미 보인다.
...
이 루프에서 `커서를 기준으로 좌/우 스택(또는 리스트) 두 개를 유지하는 editor simulation`가 설명 문장에서 실제 전이 규칙으로 바뀐다. 구현의 무게중심이 어디였는지 가장 짧게 보여 주는 블록이다.

새로 배운 것:

- 개념 문서를 다시 읽으면서, `커서를 기준으로 좌/우 스택(또는 리스트) 두 개를 유지하는 editor simulation`가 정답 공식이 아니라 상태 설계 규칙이라는 점이 또렷해졌다. 그래서 `빈 좌측 버퍼에서 B/L 명령 처리 누락` 같은 실수도 같은 자리에서 함께 막을 수 있었다.

다음:

- 이제 `빈 좌측 버퍼에서 B/L 명령 처리 누락` 같은 실수 포인트가 실제로 어디서 막히는지 fixture 전체를 돌려 확인한다.

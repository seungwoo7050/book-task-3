...
```bash
$ make -C study/Core-09-Greedy/11047/problem test
```
...
- 당시 목표: `그리디 개념 정리 — 동전 0`를 실제 코드와 연결해 마지막 설명 축을 세운다.
...
이 블록 덕분에 개념 정리가 공중에 뜨지 않는다. `내림차순 순회 누락`를 막는 방식과 `큰 동전부터 가능한 만큼 선택하는 greedy coin change`가 여기서 한 번에 맞물린다.
...
마지막 출력과 종료 조건은 사소해 보여도 최종 출력까지 닫는 부분이다. `동전 0`도 이 구간이 흐려지면 첫 구현은 맞아 보여도 최종 설명이 흔들린다.

새로 배운 것:

- 이 프로젝트를 다시 정리하면서, `그리디 개념 정리 — 동전 0`에서 말한 원리가 결국 `큰 동전부터 가능한 만큼 선택하는 greedy coin change` 한 줄로 수렴한다는 점이 선명해졌다.

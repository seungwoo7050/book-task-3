...
- `python/src/solution.py`에서 `양수/음수/1/0을 분리해 곱셈 이득이 큰 쌍을 우선 결합`를 실제 상태 전이로 옮긴다.
...
- 당시 목표: `양수/음수/1/0을 분리해 곱셈 이득이 큰 쌍을 우선 결합`를 Python 한 파일에서 바로 읽히는 상태 전이로 만든다.
...
핵심은 아이디어 이름이 아니라 순서다. 이 블록을 보면 왜 `1을 곱해버려 합이 감소하는 실수` 같은 실수가 같은 자리에서 함께 걸러지는지도 바로 드러난다.

새로 배운 것:

- 이번 구현에서 다시 확인한 건 `수 묶기 그리디 개념 정리`가 추상 설명에 머물지 않는다는 점이다. `탐욕 선택의 기준을 말로 설명하고 반례 가능성을 점검하는 연습`를 코드로 옮기려면 결국 상태 이름과 순회를 먼저 고정해야 했다.

다음:

- 이제 `1을 곱해버려 합이 감소하는 실수` 같은 실수 포인트가 실제로 어디서 막히는지 fixture 전체를 돌려 확인한다.

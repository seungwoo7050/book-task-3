...
- `python/src/solution.py`에서 `종료 시간 우선 정렬 후 가능한 회의를 순차 선택하는 activity selection`를 실제 상태 전이로 옮긴다.
...
- 당시 목표: `종료 시간 우선 정렬 후 가능한 회의를 순차 선택하는 activity selection`를 Python 한 파일에서 바로 읽히는 상태 전이로 만든다.
- 변경 단위: `python/src/solution.py`
- 처음 가설: `탐욕 선택의 기준을 말로 설명하고 반례 가능성을 점검하는 연습`라면, 핵심 자료구조와 메인 루프를 먼저 정해야 다른 분기도 자연스럽게 따라온다.
...
이 setup에서 어떤 상태를 이름 붙였는지가 뒤의 분기 전체를 결정했다. `탐욕 선택의 기준을 말로 설명하고 반례 가능성을 점검하는 연습`를 실제 코드로 바꿀 때 가장 먼저 굳는 것도 늘 이 부분이다.
...
이 루프에서 `종료 시간 우선 정렬 후 가능한 회의를 순차 선택하는 activity selection`가 설명 문장에서 실제 전이 규칙으로 바뀐다. 구현의 무게중심이 어디였는지 가장 짧게 보여 주는 블록이다.

새로 배운 것:

- `활동 선택 문제 개념 정리 — 회의실 배정`를 다시 보니, `종료 시간 우선 정렬 후 가능한 회의를 순차 선택하는 activity selection`의 핵심은 아이디어를 많이 추가하는 데 있지 않았다. `종료 시간 tie-breaker(시작 시간) 누락`를 피할 수 있는 상태를 먼저 정하는 쪽이 더 중요했다.

다음:

- 이제 `종료 시간 tie-breaker(시작 시간) 누락` 같은 실수 포인트가 실제로 어디서 막히는지 fixture 전체를 돌려 확인한다.

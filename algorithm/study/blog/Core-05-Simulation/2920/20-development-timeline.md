...
```bash
$ make -C study/Core-05-Simulation/2920/problem test
```

검증 신호:

- `Test 1: PASS`, `Test 2: PASS`, `Test 3: PASS`, `Results: 3/3 passed, 0 failed` 순서로 출력됐다.
...
- 당시 목표: `시뮬레이션 개념 정리 — 음계 판별`를 실제 코드와 연결해 마지막 설명 축을 세운다.
...
이 블록 덕분에 개념 정리가 공중에 뜨지 않는다. `오름/내림 동시 false일 때 mixed 처리 누락`를 막는 방식과 `인접 음계 차이를 검사해 ascending/descending/mixed 판정`가 여기서 한 번에 맞물린다.
...
- 마지막에 남은 교훈은 `시뮬레이션 개념 정리 — 음계 판별`가 별도 이론 노트가 아니라는 점이었다. 실제 구현에서는 `오름/내림 동시 false일 때 mixed 처리 누락`를 막는 줄이 곧 개념 설명의 중심이었다.

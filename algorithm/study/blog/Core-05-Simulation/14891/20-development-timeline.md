...
- 전파 계산 이전에 상태를 수정하는 실수
- 기어 인덱스(1-based/0-based) 혼동
- 점수 계산 비트 위치 해석 오류
...
- 당시 목표: `톱니바퀴 시뮬레이션 개념 정리`를 실제 코드와 연결해 마지막 설명 축을 세운다.
...
마지막 전환점은 이 코드였다. `톱니바퀴 시뮬레이션 개념 정리`를 붙여 읽으면, 왜 이 문제의 설명이 결국 이 줄로 수렴하는지 자연스럽게 보인다.
...
- 이 프로젝트를 다시 정리하면서, `톱니바퀴 시뮬레이션 개념 정리`에서 말한 원리가 결국 `기어 맞물림 방향 전파를 먼저 계산한 뒤 동시 회전 적용` 한 줄로 수렴한다는 점이 선명해졌다.

# 로봇 청소기: 검증, edge case, 마지막 설명 축

이제는 구현을 끝냈다고 말하는 단계가 아니라, 어떤 근거로 끝났다고 말할 수 있는지를 정리하는 단계다. 테스트 루프와 마지막 설명 축이 여기서 닫힌다.

## 구현 순서 요약

- `make -C study/Core-05-Simulation/14503/problem test`로 fixture 전체를 다시 돌린다.
- `docs/references/approach.md`의 실수 포인트를 코드 분기와 연결한다.
- `로봇 청소기 시뮬레이션 개념 정리`와 `cpp/src/solution.cpp`를 붙여 마지막 판단 기준을 고정한다.
...
이 블록을 다시 보면 `방향 회전 순서(북서남동) 혼동`를 막는 자리가 어디인지 바로 보인다. 테스트가 통과했다는 사실을 코드로 다시 설명하는 데 가장 적절한 증거다.
...
- 당시 목표: `로봇 청소기 시뮬레이션 개념 정리`를 실제 코드와 연결해 마지막 설명 축을 세운다.
...
이 블록 덕분에 개념 정리가 공중에 뜨지 않는다. `방향 회전 순서(북서남동) 혼동`를 막는 방식과 `현재 방향 기준으로 좌회전 탐색을 반복하는 로봇 청소 시뮬레이션`가 여기서 한 번에 맞물린다.
...
끝을 어떻게 닫느냐가 생각보다 중요했다. `로봇 청소기`에서는 마지막 출력 정리가 구현의 완성도를 가장 직접적으로 드러냈다.

새로 배운 것:

- 비교 구현을 함께 읽으니 `로봇 청소기 시뮬레이션 개념 정리`가 더 선명해졌다. 언어가 달라도 `현재 방향 기준으로 좌회전 탐색을 반복하는 로봇 청소 시뮬레이션`를 지탱하는 상태 규칙은 거의 바뀌지 않았다.
- 그래서 `Core-05-Simulation`의 질문인 `긴 문제 설명을 작은 상태 전이 규칙으로 어떻게 쪼갤까?`도 결과 요약이 아니라, 상태와 순서를 어떻게 붙잡는가의 문제로 읽히게 됐다.

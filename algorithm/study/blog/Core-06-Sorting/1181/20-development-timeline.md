# 단어 정렬: 검증, edge case, 마지막 설명 축

이제는 구현을 끝냈다고 말하는 단계가 아니라, 어떤 근거로 끝났다고 말할 수 있는지를 정리하는 단계다. 테스트 루프와 마지막 설명 축이 여기서 닫힌다.

## 구현 순서 요약

- `make -C study/Core-06-Sorting/1181/problem test`로 fixture 전체를 다시 돌린다.
- `docs/references/approach.md`의 실수 포인트를 코드 분기와 연결한다.
- `다중 키 정렬 개념 정리 — 단어 정렬`를 붙여 마지막 판단 기준을 고정한다.
...
- 당시 목표: `다중 키 정렬 개념 정리 — 단어 정렬`를 실제 코드와 연결해 마지막 설명 축을 세운다.
...
마지막 전환점은 이 코드였다. `다중 키 정렬 개념 정리 — 단어 정렬`를 붙여 읽으면, 왜 이 문제의 설명이 결국 이 줄로 수렴하는지 자연스럽게 보인다.
...
끝을 어떻게 닫느냐가 생각보다 중요했다. `단어 정렬`에서는 마지막 출력 정리가 구현의 완성도를 가장 직접적으로 드러냈다.

새로 배운 것:

- `다중 키 정렬 개념 정리 — 단어 정렬`를 다시 읽고 나니, 이 문제의 핵심은 `중복 제거 후 (길이, 사전순) 복합 키로 정렬`를 끝까지 흔들리지 않게 유지하는 데 있었다.

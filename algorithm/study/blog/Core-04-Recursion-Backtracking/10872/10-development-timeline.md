...
- `python/src/solution.py`에서 `재귀 또는 반복으로 N!을 누적 계산하는 기본 recursion`를 실제 상태 전이로 옮긴다.
...
```bash
$ make -C study/Core-04-Recursion-Backtracking/10872/problem run-py
```
...
- 당시 목표: `재귀 또는 반복으로 N!을 누적 계산하는 기본 recursion`를 Python 한 파일에서 바로 읽히는 상태 전이로 만든다.
...
이 부분이 풀리면서 문제는 훨씬 단순해졌다. `재귀 또는 반복으로 N!을 누적 계산하는 기본 recursion`를 지키는 데 필요한 반복 순서와 guard가 이 블록 안에서 같이 정리되기 때문이다.

새로 배운 것:

- `재귀(Recursion) 개념 정리 — 팩토리얼`를 다시 보니, `재귀 또는 반복으로 N!을 누적 계산하는 기본 recursion`의 핵심은 아이디어를 많이 추가하는 데 있지 않았다. `기저 조건 누락으로 무한 재귀`를 피할 수 있는 상태를 먼저 정하는 쪽이 더 중요했다.

다음:

- 이제 `기저 조건 누락으로 무한 재귀` 같은 실수 포인트가 실제로 어디서 막히는지 fixture 전체를 돌려 확인한다.

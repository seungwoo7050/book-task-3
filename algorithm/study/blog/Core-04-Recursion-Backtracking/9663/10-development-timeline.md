...
- `python/src/solution.py`에서 `열/대각선 점유 배열을 이용한 N-Queen backtracking`를 실제 상태 전이로 옮긴다.
...
- 당시 목표: `열/대각선 점유 배열을 이용한 N-Queen backtracking`를 Python 한 파일에서 바로 읽히는 상태 전이로 만든다.
...
문제를 단순하게 만든 건 화려한 알고리즘 이름보다 setup였다. 어떤 값을 오래 들고 갈지 정하는 순간 `열/대각선 점유 배열을 이용한 N-Queen backtracking`의 뼈대가 이미 보인다.
...
이 부분이 풀리면서 문제는 훨씬 단순해졌다. `열/대각선 점유 배열을 이용한 N-Queen backtracking`를 지키는 데 필요한 반복 순서와 guard가 이 블록 안에서 같이 정리되기 때문이다.

새로 배운 것:

- 이번 구현에서 다시 확인한 건 `N-Queen 개념 정리`가 추상 설명에 머물지 않는다는 점이다. `호출 구조를 추적하고 상태 복원 규칙을 설명하는 연습`를 코드로 옮기려면 결국 상태 이름과 순회를 먼저 고정해야 했다.

다음:

- 이제 `좌상/우상 대각선 인덱스 변환 오류` 같은 실수 포인트가 실제로 어디서 막히는지 fixture 전체를 돌려 확인한다.

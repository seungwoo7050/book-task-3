...
```bash
$ make -C study/Core-0D-MST-Topo/9372/problem test
```
...
- 당시 목표: `스패닝 트리 기초 개념 — 상근이의 여행`를 실제 코드와 연결해 마지막 설명 축을 세운다.
...
이 블록 덕분에 개념 정리가 공중에 뜨지 않는다. `테스트 케이스 반복 처리 누락`를 막는 방식과 `연결 그래프에서 최소 비행기 수는 항상 N-1임을 이용한 단순 출력`가 여기서 한 번에 맞물린다.
...
마지막 출력과 종료 조건은 사소해 보여도 최종 출력까지 닫는 부분이다. `상근이의 여행`도 이 구간이 흐려지면 첫 구현은 맞아 보여도 최종 설명이 흔들린다.

새로 배운 것:

- 마지막에 남은 교훈은 `스패닝 트리 기초 개념 — 상근이의 여행`가 별도 이론 노트가 아니라는 점이었다. 실제 구현에서는 `테스트 케이스 반복 처리 누락`를 막는 줄이 곧 개념 설명의 중심이었다.

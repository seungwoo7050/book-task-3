...
- `python/src/solution.py`에서 `최대 힙(max-heap)에 삽입/삭제 연산을 매 명령마다 처리`를 실제 상태 전이로 옮긴다.
...
```bash
$ make -C study/Core-0A-Priority-Queue/11279/problem run-py
```
...
- 당시 목표: `최대 힙(max-heap)에 삽입/삭제 연산을 매 명령마다 처리`를 Python 한 파일에서 바로 읽히는 상태 전이로 만든다.
...
```bash
$ cd study/Core-0A-Priority-Queue/11279/problem && python3 ../python/src/solution.py < data/input1.txt
```
...
문제를 단순하게 만든 건 화려한 알고리즘 이름보다 setup였다. 어떤 값을 오래 들고 갈지 정하는 순간 `최대 힙(max-heap)에 삽입/삭제 연산을 매 명령마다 처리`의 뼈대가 이미 보인다.
...
이 루프에서 `최대 힙(max-heap)에 삽입/삭제 연산을 매 명령마다 처리`가 설명 문장에서 실제 전이 규칙으로 바뀐다. 구현의 무게중심이 어디였는지 가장 짧게 보여 주는 블록이다.

새로 배운 것:

- 이번 구현에서 다시 확인한 건 `우선순위 큐 / 힙 개념 정리 — 최대 힙`가 추상 설명에 머물지 않는다는 점이다. `우선순위 큐가 필요한 상황을 식별하고 비교 기준을 일관되게 유지하는 연습`를 코드로 옮기려면 결국 상태 이름과 순회를 먼저 고정해야 했다.

다음:

- 이제 `파이썬 min-heap을 max-heap으로 변환할 때 부호 처리 실수` 같은 실수 포인트가 실제로 어디서 막히는지 fixture 전체를 돌려 확인한다.

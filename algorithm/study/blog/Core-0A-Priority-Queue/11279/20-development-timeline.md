# 최대 힙: 검증, edge case, 마지막 설명 축

뒷 절반에서는 “왜 맞는가”를 더 조밀하게 확인한다. fixture 전체를 다시 돌려 실수 포인트를 묶고, 마지막에는 개념 문서와 코드가 정확히 어디서 맞물리는지 정리한다.

## 구현 순서 요약

- `make -C study/Core-0A-Priority-Queue/11279/problem test`로 fixture 전체를 다시 돌린다.
- `docs/references/approach.md`의 실수 포인트를 코드 분기와 연결한다.
- `우선순위 큐 / 힙 개념 정리 — 최대 힙`를 붙여 마지막 판단 기준을 고정한다.
...
```bash
$ make -C study/Core-0A-Priority-Queue/11279/problem test
```
...
- 파이썬 min-heap을 max-heap으로 변환할 때 부호 처리 실수
- 빈 힙 처리 누락
- 출력 버퍼 누락
...
- 당시 목표: `우선순위 큐 / 힙 개념 정리 — 최대 힙`를 실제 코드와 연결해 마지막 설명 축을 세운다.
...
마지막 전환점은 이 코드였다. `우선순위 큐 / 힙 개념 정리 — 최대 힙`를 붙여 읽으면, 왜 이 문제의 설명이 결국 이 줄로 수렴하는지 자연스럽게 보인다.
...
마지막 출력과 종료 조건은 사소해 보여도 최종 출력까지 닫는 부분이다. `최대 힙`도 이 구간이 흐려지면 첫 구현은 맞아 보여도 최종 설명이 흔들린다.

새로 배운 것:

- `우선순위 큐 / 힙 개념 정리 — 최대 힙`를 다시 읽고 나니, 이 문제의 핵심은 `최대 힙(max-heap)에 삽입/삭제 연산을 매 명령마다 처리`를 끝까지 흔들리지 않게 유지하는 데 있었다.

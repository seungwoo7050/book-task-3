...
| 2 | `Phase 2` | Python 구현으로 첫 상태와 전이를 세운다 | `python/src/solution.py` | `가장 작은 두 묶음을 반복 병합하는 Huffman-style greedy`를 쓰려면 먼저 어떤 상태를 오래 들고 갈지 결정해야 한다 | 입력 파싱과 핵심 루프를 한 파일 안에서 먼저 닫았다 | `make -C study/Core-0A-Priority-Queue/1715/problem run-py` | `100`가 그대로 나왔다. | `python/src/solution.py`의 setup + 핵심 루프 | `N=1 예외 처리 누락`를 피하려면 아이디어보다 상태 이름과 순서가 더 중요했다 | fixture 전체 검증으로 넘어간다 |
...
| 4 | `Phase 4` | 마지막 설명 축을 코드와 붙여 둔다 | `cpp/src/solution.cpp`의 비교 구현 루프, `docs/concepts/*.md` | 마지막 글은 결과 요약이 아니라 전환점을 남겨야 한다 | Python과 C++를 나란히 두고 같은 전이가 유지되는지 다시 확인했다 | `make -C study/Core-0A-Priority-Queue/1715/problem run-cpp` | `100`가 그대로 나왔다. | `cpp/src/solution.cpp`의 비교 구현 루프 | `카드 합치기 개념 정리 — 허프만 코딩 패턴`를 구현 판단과 직접 연결해 읽게 됐다 | `00/10/20` 흐름으로 묶어 끝낸다 |

...
- `python/src/solution.py`에서 `최소 힙(min-heap)으로 0 명령 시 최솟값을 추출`를 실제 상태 전이로 옮긴다.
...
- 당시 목표: `최소 힙(min-heap)으로 0 명령 시 최솟값을 추출`를 Python 한 파일에서 바로 읽히는 상태 전이로 만든다.
...
이 루프에서 `최소 힙(min-heap)으로 0 명령 시 최솟값을 추출`가 설명 문장에서 실제 전이 규칙으로 바뀐다. 구현의 무게중심이 어디였는지 가장 짧게 보여 주는 블록이다.

새로 배운 것:

- `최소 힙 개념 정리`를 다시 보니, `최소 힙(min-heap)으로 0 명령 시 최솟값을 추출`의 핵심은 아이디어를 많이 추가하는 데 있지 않았다. `0 입력을 데이터로 오해`를 피할 수 있는 상태를 먼저 정하는 쪽이 더 중요했다.

다음:

- 이제 `0 입력을 데이터로 오해` 같은 실수 포인트가 실제로 어디서 막히는지 fixture 전체를 돌려 확인한다.

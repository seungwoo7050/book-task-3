...
- `python/src/solution.py`에서 `양끝 포인터(two pointers)로 좌우 문자를 동시에 비교`를 실제 상태 전이로 옮긴다.
...
```bash
$ make -C study/Core-00-Basics/10988/problem run-py
```
...
- 당시 목표: `양끝 포인터(two pointers)로 좌우 문자를 동시에 비교`를 Python 한 파일에서 바로 읽히는 상태 전이로 만든다.
...
문제를 단순하게 만든 건 화려한 알고리즘 이름보다 setup였다. 어떤 값을 오래 들고 갈지 정하는 순간 `양끝 포인터(two pointers)로 좌우 문자를 동시에 비교`의 뼈대가 이미 보인다.
...
핵심은 아이디어 이름이 아니라 순서다. 이 블록을 보면 왜 `인덱스 이동 순서를 잘못 두어 중앙 비교를 건너뛰는 실수` 같은 실수가 같은 자리에서 함께 걸러지는지도 바로 드러난다.

새로 배운 것:

- 개념 문서를 다시 읽으면서, `양끝 포인터(two pointers)로 좌우 문자를 동시에 비교`가 정답 공식이 아니라 상태 설계 규칙이라는 점이 또렷해졌다. 그래서 `인덱스 이동 순서를 잘못 두어 중앙 비교를 건너뛰는 실수` 같은 실수도 같은 자리에서 함께 막을 수 있었다.

다음:

- 이제 `인덱스 이동 순서를 잘못 두어 중앙 비교를 건너뛰는 실수` 같은 실수 포인트가 실제로 어디서 막히는지 fixture 전체를 돌려 확인한다.

# 수 찾기: 검증, edge case, 마지막 설명 축

이제는 구현을 끝냈다고 말하는 단계가 아니라, 어떤 근거로 끝났다고 말할 수 있는지를 정리하는 단계다. 테스트 루프와 마지막 설명 축이 여기서 닫힌다.

## 구현 순서 요약

- `make -C study/Core-07-Binary-Search-Hash/1920/problem test`로 fixture 전체를 다시 돌린다.
- `docs/references/approach.md`의 실수 포인트를 코드 분기와 연결한다.
- `이진 탐색 / 해시 개념 정리 — 수 찾기`를 붙여 마지막 판단 기준을 고정한다.
...
```bash
$ make -C study/Core-07-Binary-Search-Hash/1920/problem test
```
...
이 블록을 다시 보면 `mid 업데이트에서 무한 루프`를 막는 자리가 어디인지 바로 보인다. 테스트가 통과했다는 사실을 코드로 다시 설명하는 데 가장 적절한 증거다.
...
- 당시 목표: `이진 탐색 / 해시 개념 정리 — 수 찾기`를 실제 코드와 연결해 마지막 설명 축을 세운다.
...
이 블록 덕분에 개념 정리가 공중에 뜨지 않는다. `mid 업데이트에서 무한 루프`를 막는 방식과 `정렬된 배열에서 각 질의를 이분 탐색(binary search)으로 판정`가 여기서 한 번에 맞물린다.
...
- `이진 탐색 / 해시 개념 정리 — 수 찾기`를 다시 읽고 나니, 이 문제의 핵심은 `정렬된 배열에서 각 질의를 이분 탐색(binary search)으로 판정`를 끝까지 흔들리지 않게 유지하는 데 있었다.

...
- `python/src/solution.py`에서 `빈도 해시맵(counter)으로 카드 개수를 누적하고 질의별 출력`를 실제 상태 전이로 옮긴다.
...
- 당시 목표: `빈도 해시맵(counter)으로 카드 개수를 누적하고 질의별 출력`를 Python 한 파일에서 바로 읽히는 상태 전이로 만든다.
...
이 구간에서 입력을 어떤 상태로 바꿔 두느냐가 뒤의 복잡도를 사실상 결정했다. `Core-07-Binary-Search-Hash` 문제들이 특히 그런 경향이 강하다.
...
이 루프에서 `빈도 해시맵(counter)으로 카드 개수를 누적하고 질의별 출력`가 설명 문장에서 실제 전이 규칙으로 바뀐다. 구현의 무게중심이 어디였는지 가장 짧게 보여 주는 블록이다.

새로 배운 것:

- 개념 문서를 다시 읽으면서, `빈도 해시맵(counter)으로 카드 개수를 누적하고 질의별 출력`가 정답 공식이 아니라 상태 설계 규칙이라는 점이 또렷해졌다. 그래서 `기본값 0 처리 누락(KeyError)` 같은 실수도 같은 자리에서 함께 막을 수 있었다.

다음:

- 이제 `기본값 0 처리 누락(KeyError)` 같은 실수 포인트가 실제로 어디서 막히는지 fixture 전체를 돌려 확인한다.

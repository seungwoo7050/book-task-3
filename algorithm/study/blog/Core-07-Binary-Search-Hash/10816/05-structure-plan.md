# 숫자 카드 2 Structure Plan

이 시리즈는 `숫자 카드 2`를 읽는 흐름이 `문제 계약 -> 첫 구현 -> 검증과 마무리`로 자연스럽게 이어지도록 나눈 메모다. 파일 구조는 그대로 두되, 각 문서가 맡는 역할은 더 분명하게 보이도록 정리했다.
...
- `20`: `test.sh` 기반 검증, `approach.md`의 실수 포인트, Python finish snippet, `Counter / 해시맵 개념 정리 — 숫자 카드 2`

## 읽으면서 확인할 질문

- `빈도 해시맵(counter)으로 카드 개수를 누적하고 질의별 출력`가 실제 코드에서는 어느 상태와 반복 순서로 드러나는가?
- `make -C study/Core-07-Binary-Search-Hash/10816/problem test`가 마지막에 보장해 주는 건 정확히 무엇인가?
- `Counter / 해시맵 개념 정리 — 숫자 카드 2`를 붙여 읽으면 어떤 줄이 이 문제의 전환점으로 보이는가?

...
- `python/src/solution.py`에서 `동일 그래프에서 DFS와 BFS를 각각 수행해 방문 순서를 출력`를 실제 상태 전이로 옮긴다.
...
- 당시 목표: `동일 그래프에서 DFS와 BFS를 각각 수행해 방문 순서를 출력`를 Python 한 파일에서 바로 읽히는 상태 전이로 만든다.
- 변경 단위: `python/src/solution.py`
- 처음 가설: `그래프 표현을 고르고 방문 순서를 안정적으로 제어하는 연습`라면, 핵심 자료구조와 메인 루프를 먼저 정해야 다른 분기도 자연스럽게 따라온다.
- 실제 조치: setup에서 입력과 상태를 정리하고, 중심 루프에서 전이 순서와 guard를 함께 굳혔다.

CLI 2 (직접 Python 재실행):

```bash
$ cd study/Core-03-BFS-DFS/1260/problem && python3 ../python/src/solution.py < data/input1.txt
```
...
이 setup에서 어떤 상태를 이름 붙였는지가 뒤의 분기 전체를 결정했다. `그래프 표현을 고르고 방문 순서를 안정적으로 제어하는 연습`를 실제 코드로 바꿀 때 가장 먼저 굳는 것도 늘 이 부분이다.
...
핵심은 아이디어 이름이 아니라 순서다. 이 블록을 보면 왜 `두 탐색 간 방문 배열을 재사용해 순서가 깨지는 문제` 같은 실수가 같은 자리에서 함께 걸러지는지도 바로 드러난다.

새로 배운 것:

- 개념 문서를 다시 읽으면서, `동일 그래프에서 DFS와 BFS를 각각 수행해 방문 순서를 출력`가 정답 공식이 아니라 상태 설계 규칙이라는 점이 또렷해졌다. 그래서 `두 탐색 간 방문 배열을 재사용해 순서가 깨지는 문제` 같은 실수도 같은 자리에서 함께 막을 수 있었다.

다음:

- 이제 `두 탐색 간 방문 배열을 재사용해 순서가 깨지는 문제` 같은 실수 포인트가 실제로 어디서 막히는지 fixture 전체를 돌려 확인한다.

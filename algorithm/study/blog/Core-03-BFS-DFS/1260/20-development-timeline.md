...
이 블록을 다시 보면 `두 탐색 간 방문 배열을 재사용해 순서가 깨지는 문제`를 막는 자리가 어디인지 바로 보인다. 테스트가 통과했다는 사실을 코드로 다시 설명하는 데 가장 적절한 증거다.
...
- 당시 목표: `DFS와 BFS 비교 개념 정리`를 실제 코드와 연결해 마지막 설명 축을 세운다.
...
여기서는 `DFS와 BFS 비교 개념 정리`가 별도 해설이 아니라는 점이 분명해진다. `동일 그래프에서 DFS와 BFS를 각각 수행해 방문 순서를 출력`를 끝까지 지키는 데 필요한 최소 단위가 바로 이 블록에 남아 있다.
...
마지막 출력과 종료 조건은 사소해 보여도 최종 출력까지 닫는 부분이다. `DFS와 BFS`도 이 구간이 흐려지면 첫 구현은 맞아 보여도 최종 설명이 흔들린다.

새로 배운 것:

- 마지막에 남은 교훈은 `DFS와 BFS 비교 개념 정리`가 별도 이론 노트가 아니라는 점이었다. 실제 구현에서는 `두 탐색 간 방문 배열을 재사용해 순서가 깨지는 문제`를 막는 줄이 곧 개념 설명의 중심이었다.

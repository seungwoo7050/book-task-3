# Core-00-Basics

## 트랙 한 줄 질문

작은 입력과 조건 분기를 어떻게 안정적으로 구현하고 검증할까?

## 왜 이 순서인가

작은 문제를 안정적으로 풀지 못하면 뒤의 자료구조나 그래프 문제에서도 실수가 반복된다. 이 트랙은 구현 실수를 줄이는 최소 단위를 먼저 연습하게 한다.

## 프로젝트 카탈로그

| 순서 | 프로젝트 | 문제가 뭐였나 | 이 레포의 답 | 검증 | 상태 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| 1 | [10988](10988/README.md) | `팰린드롬인지 확인하기` | `python/src/` | `make -C study/Core-00-Basics/10988/problem test` | `verified` |
| 2 | [11053](11053/README.md) | `가장 긴 증가하는 부분 수열` | `python/src/`, `cpp/src/` | `make -C study/Core-00-Basics/11053/problem test` | `verified` |
| 3 | [16926](16926/README.md) | `배열 돌리기 1` | `python/src/` | `make -C study/Core-00-Basics/16926/problem test` | `verified` |

## 공통 읽기 순서

1. [../README.md](../README.md)에서 전체 트랙 인덱스를 확인한다.
2. [../../docs/curriculum-map.md](../../docs/curriculum-map.md)에서 이 트랙이 놓인 이유를 본다.
3. 원하는 프로젝트 README에서 6문답을 먼저 읽고 `problem/ -> 구현 -> docs/ -> notion/` 순서로 내려간다.

## 포트폴리오 관점 메모

기초 트랙을 포트폴리오에 옮길 때는 화려한 문제보다 '입력 해석, 상태 정의, 검증 명령'을 얼마나 분리해 보여 주는지가 더 중요하다.

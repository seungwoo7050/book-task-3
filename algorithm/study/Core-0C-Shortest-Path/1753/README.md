...
| :--- | :--- |
| 상태 | `verified` |
| 문제 배경 | `BOJ 1753 최단경로` 문제를 현재 학습 구조에서 설명 가능한 풀이 프로젝트로 다시 정리한 항목 |
...
## 이 레포의 답
- 한 줄 답: `단일 시작점에서 모든 정점까지 Dijkstra 최단거리 계산`
...
## 어떻게 검증하나
- 정식 검증: `make -C study/Core-0C-Shortest-Path/1753/problem test`
- 대표 실행: `make -C study/Core-0C-Shortest-Path/1753/problem run-py`
- 비교 실행: `make -C study/Core-0C-Shortest-Path/1753/problem run-cpp`

...
이 문서는 `계산 기하 실습` 프로젝트를 읽을 때 어떤 판단 기준으로 구현을 따라가면 좋은지 정리한 짧은 안내서다. 정답 코드 자체보다, 어떤 상태를 먼저 고정하고 어떤 연산을 나중에 확인해야 하는지 설명하는 데 목적이 있다.

## 이번 프로젝트에서 특히 볼 것

- 학습 초점: 계산 기하 실습의 핵심 아이디어를 작은 실험과 자동 검증으로 다시 설명하는 연습

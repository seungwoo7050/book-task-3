# 0x18 NP-완전성 실습 학습 프로젝트

| 항목 | 내용 |
| :--- | :--- |
| 상태 | `verified` |
| 문제 배경 | `NP-완전성 실습` 개념을 `CLRS Ch 34` 맥락에서 실행 가능한 입출력 실험으로 다시 정리한 심화 프로젝트 |
| 정식 검증 | `make -C study/Advanced-CLRS/0x18-np-completeness-lab/problem test` |
...
## 이 레포의 답
- 한 줄 답: `python/src/solution.py`에서 `NP-완전성 실습` 핵심 절차를 실행 가능한 실험으로 재현
- 공개 답안 위치: `python/src/`
...
## 어떻게 검증하나
- 정식 검증: `make -C study/Advanced-CLRS/0x18-np-completeness-lab/problem test`
- 대표 실행: `make -C study/Advanced-CLRS/0x18-np-completeness-lab/problem run-py`

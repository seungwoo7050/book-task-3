...
| 개념 | CLRS 참조 | 설명 |
|------|-----------|------|
| Ford-Fulkerson | Ch 26.2 | 증가 경로 반복 |
| Edmonds-Karp | Ch 26.2 | BFS 기반 Ford-Fulkerson, $O(VE^2)$ |
| 잔여 그래프 | Ch 26.2 | 남은 용량 + 역간선 |
| 최대 유량-최소 컷 | Ch 26.2 | max-flow = min-cut |
| 증가 경로 | Ch 26.2 | $s \to t$ 잔여 용량 양수 경로 |

## 다시 연결해 볼 질문

- `네트워크 플로우`에서 쓴 개념이 같은 트랙의 다음 문제에서 어떻게 더 어려워지는가?
- `네트워크 플로우의 핵심 아이디어를 작은 실험과 자동 검증으로 다시 설명하는 연습`를 다른 자료구조나 다른 언어 구현으로 옮기면 무엇이 달라지는가?
- 이번에 정리한 용어와 판단 기준을 README 한 단락으로 축약하면 무엇이 남는가?

## 한 줄 정리 후보

- `네트워크 플로우`를 다시 설명할 때는 문제 이름보다 `네트워크 플로우의 핵심 아이디어를 작은 실험과 자동 검증으로 다시 설명하는 연습`를 먼저 말한다.

...
| 개념 | CLRS 참조 | 설명 |
|------|-----------|------|
| Extended GCD | Ch 31.2 | $ax + by = \gcd(a,b)$ |
| 모듈러 역원 | Ch 31.4 | $a^{-1} \bmod m$ |
| 중국인 나머지 정리 | Ch 31.5 | 연립 합동식 해 |
| RSA | Ch 31.7 | 공개키 암호 시스템 |
| 오일러 정리 | Ch 31.6 | $a^{\phi(n)} \equiv 1 \pmod{n}$ |

## 다시 연결해 볼 질문

- `정수론 실습`에서 쓴 개념이 같은 트랙의 다음 문제에서 어떻게 더 어려워지는가?
- `정수론 실습의 핵심 아이디어를 작은 실험과 자동 검증으로 다시 설명하는 연습`를 다른 자료구조나 다른 언어 구현으로 옮기면 무엇이 달라지는가?
- 이번에 정리한 용어와 판단 기준을 README 한 단락으로 축약하면 무엇이 남는가?

## 한 줄 정리 후보

- `정수론 실습`를 다시 설명할 때는 문제 이름보다 `정수론 실습의 핵심 아이디어를 작은 실험과 자동 검증으로 다시 설명하는 연습`를 먼저 말한다.
...
- 다음 프로젝트: [`../../0x18-np-completeness-lab/README.md`](../../0x18-np-completeness-lab/README.md) (NP-완전성 실습)

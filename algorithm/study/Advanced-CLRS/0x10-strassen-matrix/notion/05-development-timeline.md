...
- 현재 기준 경로: `study/Advanced-CLRS/0x10-strassen-matrix`
...
자체 제작 입출력 문제. `problem/` 내 fixture 생성.

### 단계 2: Python 구현

naive + strassen 동시 구현. split/combine/pad 유틸리티.
...
- `Strassen 행렬 곱셈`에서는 `Strassen 행렬 곱셈의 핵심 아이디어를 작은 실험과 자동 검증으로 다시 설명하는 연습`를 실제 명령과 문서 흐름으로 다시 밟아 볼 수 있어야 학습 기록으로서 가치가 생긴다.

...
근사 알고리즘의 출력이 테스트 fixture와 일치하려면 동률 시 선택 순서가 결정적이어야 함. 인덱스 기준 정렬 사용.
...
- `근사 알고리즘 실습`는 `근사 알고리즘 실습의 핵심 아이디어를 작은 실험과 자동 검증으로 다시 설명하는 연습`를 연습하는 프로젝트라서, 작은 경계 조건 하나가 전체 상태 전이를 망가뜨릴 수 있다.

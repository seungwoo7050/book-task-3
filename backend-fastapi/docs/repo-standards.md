...
루트 `README.md`는 첫 화면에서 아래 네 가지를 바로 보여줘야 합니다.
...
루트 README의 커리큘럼 표는 최소한 아래 열을 포함합니다.

```text
트랙 | 경로 | 문제 요약 | 답 형태 | 검증 위치
```

각 lab 또는 capstone의 `README.md`는 canonical problem statement를 반복 복붙하지 않고, 아래 순서의 요약 인덱스로 작성합니다.
...
프로젝트 README는 "무엇을 배운다"보다 먼저 "무슨 문제를 풀고 어떤 답을 냈는가"를 드러내야 합니다. 상세 제약과 성공 기준은 `problem/README.md`에 남기고, README에는 2~3문장 요약만 둡니다.

`fastapi/README.md`, `docs/README.md`, `notion/README.md`의 역할도 섞지 않습니다.

- `fastapi/README.md`: 실행과 검증 진입점
- `docs/README.md`: 설계 설명과 개념 지도
- `notion/README.md`: 학습 로그와 재현 순서

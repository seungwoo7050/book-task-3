...
def _latest_token(email: str) -> str:
    response = httpx.get(
        "http://127.0.0.1:8131/api/v1/internal/auth/debug/mailbox/latest",
        params={"email": email},
        timeout=5.0,
    )
...
def test_v2_system_flow_and_notification_recovery() -> None:
    with compose_stack() as project_name:
        ...
        with connect(f"ws://127.0.0.1:8014/api/v1/platform/ws/notifications?access_token={access_token}") as websocket:
            project = owner.post(

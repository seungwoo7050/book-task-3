...
def _heuristic_verdict(claim_text: str, doc_text: str) -> str:
    ...
    if coverage >= 0.25 and claim_has_negative == doc_has_negative:
        return "support"
    if coverage >= 0.25 and claim_has_positive and doc_has_positive and not claim_has_negative and not doc_has_negative:
        return "support"
    if coverage >= 0.2 and claim_has_negative and doc_has_positive and not doc_has_negative:
        return "contradict"
    if coverage >= 0.2 and claim_has_positive and doc_has_negative and not claim_has_negative:
        return "contradict"
    if coverage >= 0.2:
        return "support"
    return "not_found"
...
def verify_claims_with_trace(
    ...
    for claim in claims:
        ...
        category_hint = DOMAIN_CATEGORY_HINT.get(claim.domain)
        docs = retriever.search(
            claim.statement,
            top_k=top_k,
            category=category_hint,
            backend=backend,
            bundle_id=kb_bundle_id,
        )
        if docs:
            retrieval_hits += 1
        ...
        claim_results.append(
            EvidenceClaimResult(
                ...
                retrieval_trace=RetrievalTrace(
                    ...
                    backend=backend,
                    category_hint=category_hint or claim.domain,
...
def verify_claims(session: Session, claims: list[Claim], top_k: int = 3, kb_bundle_id: str | None = None) -> EvidenceResult:
    result, _ = verify_claims_with_trace(session, claims, top_k=top_k, kb_bundle_id=kb_bundle_id)
    return result

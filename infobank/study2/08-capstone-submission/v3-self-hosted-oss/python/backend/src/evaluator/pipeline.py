...
class EvaluationPipeline:
    ...
    def _existing_evaluation(
        ...
        stmt = select(Evaluation).where(
            ...
            Evaluation.kb_bundle_id == kb_bundle_id,
        )
    ...
    def evaluate_turn(
        ...
        *,
        ...
        effective_bundle_id = kb_bundle_id or (run.kb_bundle_id if run is not None else None)
        ...
        if allow_cache:
            cached = self._existing_evaluation(
                turn_id,
                evaluator_version,
                prompt_version,
                kb_version,
        ...
        try:
            ...
            if has_critical_rule(rule_results):
                ...
                evidence_result, evidence_attempts = verify_claims_with_trace(
                    self.session,
                    claims,
                    top_k=3,
                    kb_bundle_id=effective_bundle_id,
                )
        ...
        *,
        ...
        return [
            self.evaluate_turn(
                ...
                run=run,
                retrieval_version=retrieval_version,
                kb_bundle_id=run.kb_bundle_id if run is not None else None,
            )
...
def serialize_evaluation(evaluation: Evaluation) -> dict[str, object]:
    return {
        ...
        "kb_bundle_id": evaluation.kb_bundle_id,

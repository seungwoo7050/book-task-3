...
def run_golden_set(payload: GoldenSetRunRequest, session: Session = Depends(get_session)) -> dict[str, object]:
    ...
    run = create_evaluation_run(
        ...
        dataset_name=payload.dataset or "golden-set",
    ...
    try:
        for row in rows:
            conversation = Conversation(
                ...
                prompt_version=payload.prompt_version or run.prompt_version,
                kb_version=payload.kb_version or run.kb_version,
            )
            ...
            evaluation.assertion_result = dumps_json(assertion.to_dict())

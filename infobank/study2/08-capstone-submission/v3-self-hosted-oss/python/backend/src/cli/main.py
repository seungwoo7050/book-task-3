...
from db.models import Conversation, Evaluation, EvaluationRun, GoldenSet, Turn
...
def init_db_command() -> None:
    ...
    console.print("[green]Database initialized and sample assets bootstrapped[/green]")
...
def worker(once: bool = typer.Option(False, "--once")) -> None:
    init_db()
    run_worker_loop(poll_seconds=load_settings().worker_poll_seconds, once=once)
    if once:
        console.print("[green]Worker finished one polling cycle[/green]")
...
@app.command("replay")
def replay(
    fixture: str = typer.Option("backend/data/fixtures/replay_sessions.yaml", "--fixture"),
    evaluator_version: str = typer.Option("eval-v1", "--evaluator-version"),
    prompt_version: str = typer.Option("v1.0", "--prompt-version"),
    kb_version: str = typer.Option("v1.0", "--kb-version"),
) -> None:
    init_db()
    with session_scope() as session:
...
def evaluate(
    ...
    dataset: str = typer.Option("golden-set", "--dataset"),
    retrieval_version: str = typer.Option("retrieval-v1", "--retrieval-version"),
) -> None:
    init_db()
    ...
    with session_scope() as session:
        ...
        if turn:
            ...
            for row in golden_rows:
                ...
                evaluation.assertion_result = json.dumps(assertion.to_dict(), ensure_ascii=False)
...
def compare(baseline: str, candidate: str, dataset: str = typer.Option("golden-set", "--dataset")) -> None:
    with session_scope() as session:
        base_rows = list(
            session.scalars(
                ...
                .where(EvaluationRun.run_label == baseline, EvaluationRun.dataset_name == dataset)
            ).all()
        )
        cand_rows = list(
            session.scalars(
                select(Evaluation)
                .join(EvaluationRun, EvaluationRun.id == Evaluation.run_id)
                .where(EvaluationRun.run_label == candidate, EvaluationRun.dataset_name == dataset)
            ).all()
        )
    ...
    def _assertion_counts(rows: list[Evaluation]) -> tuple[int, int]:
        pass_count = 0
        fail_count = 0
        for item in rows:
            assertion = loads_json(item.assertion_result, {})
    ...
    base_pass, base_fail = _assertion_counts(base_rows)
    cand_pass, cand_fail = _assertion_counts(cand_rows)
    ...
    table.add_row(
        ...
        str(sum(1 for item in cand_rows if item.is_critical) - sum(1 for item in base_rows if item.is_critical)),
    )
    table.add_row("pass_count", str(base_pass), str(cand_pass), str(cand_pass - base_pass))
    table.add_row("fail_count", str(base_fail), str(cand_fail), str(cand_fail - base_fail))
    console.print(table)

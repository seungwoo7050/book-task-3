...
def load_settings() -> Settings:
    ...
    return Settings(
        db_url=os.getenv("QUALBOT_DATABASE_URL", os.getenv("QUALBOT_DB_URL", "sqlite:///./backend/data/qualbot.db")),
        session_secret=os.getenv("QUALBOT_SESSION_SECRET", "qualbot-dev-session-secret"),
        session_max_age_seconds=int(os.getenv("QUALBOT_SESSION_MAX_AGE_SECONDS", "604800")),
        admin_email=os.getenv("QUALBOT_ADMIN_EMAIL", "admin@example.com"),
        admin_password=os.getenv("QUALBOT_ADMIN_PASSWORD", "change-me-admin"),
        storage_root=os.getenv("QUALBOT_STORAGE_ROOT", "backend/data/storage"),
        worker_poll_seconds=float(os.getenv("QUALBOT_WORKER_POLL_SECONDS", "1.0")),

...
from core.types import EvidenceResult, LLMJudgment, ProviderAttempt, RuleResult
...
def _llm_judge(
    ...
    rule_results: list[RuleResult],
    evidence_result: EvidenceResult,
) -> tuple[LLMJudgment, list[ProviderAttempt]]:
    try:
    ...
    provider_name = next((attempt.provider for attempt in attempts if attempt.succeeded), "provider")
    model_name = next((attempt.model for attempt in attempts if attempt.succeeded), "provider-model")
...
def judge_response(
    ...
    judgment, _ = judge_response_with_trace(user_message, assistant_response, rule_results, evidence_result)
    return judgment

# 01-selection-rubric-and-eval-contract 답안지

이 문서는 문제지를 다시 넘기지 않고도 해답을 재구성할 수 있도록, 실제 구현 파일과 테스트만 기준으로 정리한 답안지다.

## 한 줄 해답

시작 위치의 구현을 완성해 어떤 추천이 pass인지 fail인지 문서만으로 설명할 수 있다, 후속 버전 비교가 같은 기준을 사용한다는 점이 분명해진다, 학생이 자기 프로젝트에 맞는 rubric을 설계할 출발점을 얻는다를 한 흐름으로 설명하고 검증한다. 핵심은 `evaluateOfflineCases`와 `toolCategorySchema`, `transportSchema` 흐름을 구현하고 테스트를 통과시키는 것이다.
...
- 첫 진입점은 `../projects/01-mcp-recommendation-demo/capstone/v0-initial-demo/node/src/services/eval-service.ts`이고, 여기서 `evaluateOfflineCases`와 `toolCategorySchema` 흐름을 먼저 붙잡은 뒤 나머지 파일로 확장한다.

## 코드 워크스루

- `../projects/01-mcp-recommendation-demo/capstone/v0-initial-demo/node/src/services/eval-service.ts`: `evaluateOfflineCases`가 핵심 흐름과 상태 전이를 묶는다.
- `../projects/01-mcp-recommendation-demo/capstone/v0-initial-demo/shared/src/contracts.ts`: `toolCategorySchema`, `transportSchema`, `platformSchema`, `reasonTypeSchema`가 핵심 흐름과 상태 전이를 묶는다.
- `../projects/01-mcp-recommendation-demo/capstone/v0-initial-demo/shared/src/eval.ts`: `rawEvalCases`, `offlineEvalCases`가 핵심 흐름과 상태 전이를 묶는다.
...
- `../projects/01-mcp-recommendation-demo/capstone/v0-initial-demo/shared/package.json`: 실행 명령과 검증 경로를 고정하는 설정 파일이다.

## 정답을 재구성하는 절차

1. `../projects/01-mcp-recommendation-demo/capstone/v0-initial-demo/node/src/services/eval-service.ts`를 먼저 열어 입력과 상태 전이의 기준점을 잡는다.

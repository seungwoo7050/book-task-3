import { catalogSeeds, offlineEvalCases } from "@study1-v3/shared";
import type { UsageEvent } from "@study1-v3/shared";
...
async function main() {
  ...
  const settings = buildDefaultInstanceSettings("seed");
  ...
  await db.insert(users).values([
    ...
    },
    {
      ...
      email: "operator@study1.local",
      role: "operator",
      isActive: true,
      payload: {
        id: "user-operator-seed",
        email: "operator@study1.local",
        name: "Study1 Operator",
        role: "operator",
        isActive: true,
  ...
  console.log(
    `Seeded ${catalogSeeds.length} catalog entries, ${offlineEvalCases.length} eval cases, users, settings, usage signals, feedback, experiments, and release candidates.`
  );
}

...
export function runCompare(
  ...
  const rows = cases.map((item) => {
    ...
    return {
      ...
      baselineScoreValue: baseline.topCandidates[0]?.score ?? 0,
      candidateScoreValue: candidate.topCandidates[0]?.score ?? 0
    };
  });
  ...
  const baselineAverageScore =
    rows.reduce((total, row) => total + row.baselineScoreValue, 0) / rows.length;
  const candidateAverageScore =
    rows.reduce((total, row) => total + row.candidateScoreValue, 0) / rows.length;

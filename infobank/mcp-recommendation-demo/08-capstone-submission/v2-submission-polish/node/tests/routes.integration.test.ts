...
describe("route contracts", () => {
  it("returns seeded catalog, compare results, and release gate artifacts when DB is prepared", async () => {
    ...
    const releaseCandidatesResponse = await app.inject({
      method: "GET",
      url: "/api/release-candidates"
    });

    expect(releaseCandidatesResponse.statusCode).toBe(200);
    const releaseCandidatesBody = releaseCandidatesResponse.json() as {
      items: Array<{ id: string }>;
    };
    expect(releaseCandidatesBody.items[0]?.id).toBe("rc-release-check-bot-1-5-0");

    const compatibilityResponse = await app.inject({
      method: "POST",
      url: "/api/compatibility/run",
      payload: {
    ...
    const compatibilityBody = compatibilityResponse.json() as {
      passed: boolean;
    };
    expect(compatibilityBody.passed).toBe(true);

    const gateResponse = await app.inject({
      method: "POST",
      url: "/api/release-gate/run",
      payload: {
        releaseCandidateId: "rc-release-check-bot-1-5-0"
      }
    });
    ...
    const gateBody = gateResponse.json() as {
      passed: boolean;
      metrics: { uplift: number };
    };
    expect(gateBody.passed).toBe(true);
    expect(gateBody.metrics.uplift).toBeGreaterThanOrEqual(0.02);

    const artifactResponse = await app.inject({
      method: "POST",
      url: "/api/submission/export",
      payload: {
        releaseCandidateId: "rc-release-check-bot-1-5-0"
      }
    });
    ...
    expect(artifactBody.content).toContain("## Release Notes");

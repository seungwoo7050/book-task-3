...
def test_escalation_notice_warning():
    results = evaluate_rules(
        user_message="환불 거절됐는데 분쟁 접수하고 싶어요",
        assistant_response="다시 확인해 보겠습니다.",
        rules_dir="backend/rules",
    )
    assert any(item.failure_type == "ESCALATION_MISS" for item in results)

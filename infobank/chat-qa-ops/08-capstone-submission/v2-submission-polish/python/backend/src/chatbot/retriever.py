...
class Retriever:
    ...
    def _search_keyword_v1(self, query: str, top_k: int, category: str | None) -> list[RetrievedDoc]:
        docs = self._all_docs()
    ...
    def _search_keyword_v2(self, query: str, top_k: int, category: str | None) -> list[RetrievedDoc]:
        docs = self._all_docs()
    ...
    def search(
        ...
        if chosen == "keyword":
            if retrieval_version == "retrieval-v1":
                return self._search_keyword_v1(query, top_k=top_k, category=category)
            return self._search_keyword_v2(query, top_k=top_k, category=category)
        if chosen == "chroma":

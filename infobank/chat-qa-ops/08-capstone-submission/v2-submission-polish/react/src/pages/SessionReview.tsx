...
export function SessionReviewPage() {
  ...
  useEffect(() => {
    if (!selected) return;
    apiGet<{ conversation: Conversation; turns: Turn[] }>(`/api/conversations/${selected}`)
      .then((result) => {
        ...
        <div className="card">
          {conversationMeta && (
            <div className="key-value">
              <span>세션 {conversationMeta.id.slice(0, 8)}</span>
            ...
            <div key={turn.id} className="chat-turn">
              ...
              <div className="trace-grid">
                <div className="inset-card">
                  ...
                  <div>docs: {turn.retrieved_doc_ids.join(", ") || "-"}</div>
                </div>
                <div className="inset-card">
                  <strong>Lineage</strong>
                  <div>run={turn.evaluation?.lineage.run_label ?? "-"}</div>
                  <div>dataset={turn.evaluation?.lineage.dataset ?? "-"}</div>

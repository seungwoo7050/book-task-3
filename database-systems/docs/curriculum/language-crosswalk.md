...
| `python/database-internals/projects/05-mvcc` | `go/database-internals/projects/09-mvcc` | `Database Internals` | Python에서는 MVCC로 바로 가지만, Go에서는 그 전에 B+Tree와 query scan을 한 단계 더 거칩니다. |
| 해당 없음 | `go/database-internals/projects/08-btree-index-and-query-scan` | `Database Internals` | Python 입문 트랙에는 없는 Go-only deep dive입니다. buffer pool 뒤의 read-path 상위 계층을 따로 읽기 좋습니다. |

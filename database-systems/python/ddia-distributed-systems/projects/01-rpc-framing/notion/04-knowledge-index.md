...
## 다시 볼 파일
- `../src/rpc_framing/core.py`: framing decode, correlation id, pending map, timeout handling이 한 파일 안에서 이어집니다.
- `../src/rpc_framing/__main__.py`: client/server round trip을 바로 확인하는 데모 진입점입니다.
- `../tests/test_rpc_framing.py`: split chunk, concurrent call, timeout/error propagation을 검증합니다.
- `../docs/concepts/pending-map.md`: 동시 요청이 많아질 때 pending map이 왜 필요한지 먼저 복기할 때 좋습니다.
...
- 데모가 보여 주는 장면: client/server round trip 결과를 그대로 출력합니다.

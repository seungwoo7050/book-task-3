...
### Python은 HTTP boundary를 thin adapter로만 둔다
- 관련 파일: `../src/clustered_kv/app.py`, `../src/clustered_kv/core.py`
- 판단: Python 버전의 `create_app`은 FastAPI를 core 로직 위에 얇게 얹고, 테스트에서는 `TestClient`로 boundary만 검증합니다.
...
- 정적 topology라는 의도적 단순화는 “무엇을 아직 하지 않았는가”를 솔직하게 설명하는 좋은 장치입니다.
- Python 버전은 FastAPI를 얇은 경계로만 사용했다는 점, Go 버전은 core 로직만으로 self-contained cluster를 보여 준다는 점이 각각 좋은 설명 포인트입니다.

...
- Python 버전에서는 FastAPI 경계에서 PUT/GET round trip까지 확인되어야 합니다.
...
## 데모에서 바로 확인할 장면
- Go 데모는 `alpha` write 후 shard ID, follower ID, 읽은 value, 성공 여부를 함께 출력합니다. Python 데모는 FastAPI `PUT/GET /kv/alpha` round trip JSON을 바로 보여 줍니다.

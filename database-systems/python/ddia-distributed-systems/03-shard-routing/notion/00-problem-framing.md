...
## 먼저 열어 둘 파일
- `../src/shard_routing/core.py`: consistent hash ring, virtual node 생성, routing 계산이 모두 들어 있습니다. 이 구현은 SHA-256 기반 hash를 사용합니다.
- `../tests/test_shard_routing.py`: empty/single node, distribution, rebalance, batch routing을 검증합니다.
- `../src/shard_routing/__main__.py`: 몇 개의 sample key가 어느 node로 가는지 빠르게 보여 주는 데모입니다.
...
## 데모에서 바로 확인할 장면
- Go 데모는 `alpha`, `beta`, `gamma`가 어느 node로 가는지 출력합니다. Python 데모는 `k1`~`k4` batch routing 결과를 그대로 print합니다.

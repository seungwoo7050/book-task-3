...
## 왜 이 프로젝트를 하는가
01~04 단계까지는 “현재 값”을 다루는 저장 구조를 배웠다면, 05에서는 “같은 key를 서로 다른 트랜잭션이 서로 다른 시점에서 어떻게 보느냐”를 다룹니다. 목표는 production-grade MVCC를 만드는 것이 아니라, snapshot isolation의 핵심 규칙을 직접 구현하고 테스트로 재현하는 것입니다.
...
- `t2` 시작
- `t3`가 `x=200`을 commit
- `t2`는 여전히 100을 읽어야 합니다.

### 시나리오 3. write-write conflict
- `t1`과 `t2`가 둘 다 `x`를 수정
- `t1`이 먼저 commit
- `t2` commit은 `write-write conflict`로 실패해야 합니다.

### 시나리오 4. GC
- `v1`, `v2`, `v3`가 차례로 쌓인 뒤 GC를 수행
...
## 먼저 열어 둘 파일
- `../src/mvcc_lab/core.py`: `Version`, `VersionStore`, `Transaction`, `TransactionManager` 구현 전체가 있습니다.
- `../tests/test_mvcc.py`: 각 규칙이 어떤 입력 시나리오로 검증되는지 바로 볼 수 있습니다.
- `../src/mvcc_lab/__main__.py`: 가장 짧은 데모 진입점입니다.
- `../docs/concepts/snapshot-visibility.md`: visibility 규칙을 읽기 전에 먼저 맞추기 좋습니다.
...
## 데모에서 바로 확인할 장면
- `PYTHONPATH=src python -m mvcc_lab`를 실행했을 때 아래 한 줄이 나오면 read-your-own-write 경로는 맞습니다.

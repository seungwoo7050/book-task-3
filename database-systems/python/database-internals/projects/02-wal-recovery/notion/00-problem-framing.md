...
- 이전 단계: 01 Mini LSM Store
- 다음 단계: 03 Index Filter
...
## 먼저 열어 둘 파일
- `../src/wal_recovery/store.py`: WAL append, replay, flush rotation이 한 파일 안에서 어떤 순서로 이어지는지 확인합니다.
- `../src/wal_recovery/__main__.py`: restart recovery가 실제 출력으로 드러나는 가장 짧은 데모 진입점입니다.
- `../tests/test_wal_recovery.py`: corruption, truncation, reopen recovery, WAL rotation이 어디서 깨지는지 바로 확인합니다.
- `../docs/concepts/wal-record-format.md`: record byte layout과 checksum이 복구 규칙에 어떤 영향을 주는지 먼저 복기합니다.
...
## 데모에서 바로 확인할 장면
- `name`, `city`를 쓴 뒤 store를 닫고 다시 열어 복원된 값을 JSON 형태로 보여 줍니다.

...
## 다시 볼 파일
- `../src/wal_recovery/store.py`: WAL append, checksum 검증, replay stopping rule, flush rotation이 한곳에 모여 있습니다.
- `../src/wal_recovery/__main__.py`: restart recovery가 어떤 출력으로 보이는지 바로 확인하는 데모 진입점입니다.
- `../tests/test_wal_recovery.py`: corruption, truncation, reopen recovery, WAL rotation을 검증합니다.
- `../docs/concepts/recovery-policy.md`: 손상된 WAL을 어디까지 신뢰하고 어디서 멈출지 먼저 복기할 때 좋습니다.
...
- 데모가 보여 주는 장면: `name`, `city`를 쓴 뒤 store를 닫고 다시 열어 복원된 값을 JSON 형태로 보여 줍니다.

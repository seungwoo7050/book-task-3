# 04 Raft Lite — Structure Outline
...
- `20-chronology-core-invariants.md`: `Tick`와 `startElection`가 실제로 invariant를 고정하는 구간
...
1. `10-chronology-scope-and-surface.md`
   ...
   코드 앵커: `TestLeaderElection`, `Tick`
   CLI: `find internal tests cmd -type f | sort`, `rg -n "^func Test" tests`

2. `20-chronology-core-invariants.md`
   핵심 invariant가 `Tick`와 `startElection` 사이에서 어떻게 고정되는지 보여 준다.
   코드 앵커: `Tick`, `startElection`
   CLI: `rg -n "^(type|func) " internal cmd`, `rg -n "Tick|startElection" internal cmd`

3. `30-chronology-verification-and-boundaries.md`
   테스트와 demo를 모두 남겨, pass 신호와 공개 표면을 구분해 설명한다.
   코드 앵커: `TestLogReplicationAndCommit`, `main.go`
   CLI: GOWORK=off go test ./...; GOWORK=off go run ./cmd/raft-lite

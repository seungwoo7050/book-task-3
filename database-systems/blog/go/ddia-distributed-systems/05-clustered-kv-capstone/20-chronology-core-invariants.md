# 20 05 Clustered KV Capstone에서 진짜 중요한 상태 전이만 붙잡기
...
### Session 1 — RouteShard에서 invariant가 잠기는 지점 보기

이 구간에서 먼저 붙잡으려 한 것은 `RouteShard`가 어떤 입력을 받아 어떤 상태를 고정하는지 분해하는 것이었다. 처음 읽을 때는 `RouteShard` 하나를 이해하면 나머지 흐름도 거의 자동으로 따라올 거라고 생각했다.

그런데 `rg -n "RouteShard|shardRing" internal cmd`로 핵심 함수 위치를 다시 잡고, `RouteShard`가 문제 정의의 첫 번째 bullet과 정확히 맞물리는지 확인했다. 특히 `RouteShard` 안에서 상태가 한 번에 굳는지, 아니면 보조 구조로 넘겨지는지가 프로젝트의 설명 밀도를 갈랐다.
...
`RouteShard`는 이 프로젝트에서 규칙이 가장 먼저 굳는 지점을 보여 준다. 테스트가 요구한 첫 번째 조건이 실제 코드 규칙으로 바뀌는 순간을 여기서 확인할 수 있었다.
...
### Session 2 — shardRing로 같은 규칙 다시 확인하기

여기서 가장 먼저 확인한 것은 `shardRing`가 `RouteShard`와 어떤 짝을 이루는지 확인한다. 처음에는 `shardRing`는 단순 보조 함수일 거라고 생각했다.

하지만 실제로는 두 번째 앵커를 읽고 나니, 실제로는 `RouteShard`가 만든 상태를 외부에서 관찰 가능하게 만드는 규칙이 여기 있었다. 결정적으로 방향을 잡아 준 신호는 `shardRing`는 테스트의 뒤쪽 시나리오를 설명하는 열쇠였다.

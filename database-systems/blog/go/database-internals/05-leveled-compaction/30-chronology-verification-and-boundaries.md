# 30 05 Leveled Compaction를 다시 돌려 보며 검증 신호와 경계를 정리하기
...
`TestKWayMergeDropsTombstonesAtDeepestLevel`는 구현의 뒷부분에서 생길 수 있는 붕괴 지점을 문장보다 정확하게 고정한다. pass 숫자보다 더 중요했던 건, 어떤 경계가 계속 회귀 테스트로 남아 있느냐였다.
...
그런데 `GOWORK=off go run ./cmd/leveled-compaction`를 다시 실행해 마지막 한 줄을 확인하고, README의 `한계와 확장` bullet과 나란히 읽었다. 특히 demo 핵심 줄: `pear=green`라는 출력이 마지막 확인 지점이 됐다.

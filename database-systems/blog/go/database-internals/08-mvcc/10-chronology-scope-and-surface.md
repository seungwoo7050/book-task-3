...
이번 세션의 목표는 `08 MVCC`가 어떤 invariant를 먼저 고정하는 슬롯인지 파악하는 것이었다. 초기 가설은 구현이 너무 작아서 단순 API 연습에 가까울 거라고 봤다.
...
그런데 가장 큰 구현 파일인 `database-systems/go/database-internals/projects/08-mvcc/internal/mvcc/mvcc.go`를 먼저 읽고, 테스트가 요구한 상태 전이가 정말 이 파일 안에서 닫히는지 확인했다. 특히 `Begin` 같은 이름이 초기에 바로 보이면 write path의 중심이 선명해진다.

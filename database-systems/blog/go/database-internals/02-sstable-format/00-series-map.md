# 02 SSTable Format 시리즈 맵

이 시리즈는 Database Internals 트랙의 2번째 프로젝트 `02 SSTable Format`를 따라간다. immutable SSTable 파일 형식, sparse key index, footer metadata를 구현해 on-disk lookup의 기본을 고정합니다. 기능 목록보다 먼저, 어떤 순서로 경계를 고정했는지 읽는 쪽에 무게를 두었다.

...
- `20-chronology-core-invariants.md`: `SSTable`와 `MiniLSMStore`가 실제로 invariant를 고정하는 구간
...
1. `10-chronology-scope-and-surface.md`
   ...
   코드 앵커: `test_put_and_get`, `SSTable`
   CLI: `find src tests -type f | sort`, `rg -n "^def test_" tests`

2. `20-chronology-core-invariants.md`
   핵심 invariant가 `SSTable`와 `MiniLSMStore` 사이에서 어떻게 고정되는지 보여 준다.
   코드 앵커: `SSTable`, `MiniLSMStore`
   CLI: `rg -n "^(class|def) " src`, `rg -n "SSTable|MiniLSMStore" src`

3. `30-chronology-verification-and-boundaries.md`
   테스트와 demo를 모두 남겨, pass 신호와 공개 표면을 구분해 설명한다.
   코드 앵커: `test_persistence_after_reopen`, `__main__.py`
   CLI: PYTHONPATH=src .venv/bin/python -m pytest; PYTHONPATH=src .venv/bin/python -m mini_lsm_store

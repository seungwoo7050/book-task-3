# 20 08 MVCC의 핵심 invariant를 코드에서 고정하기
...
- 당시 목표:
  `Begin`가 어떤 입력을 받아 어떤 상태를 고정하는지 분해한다.
- 변경 단위:
  `database-systems/go/database-internals/projects/08-mvcc/internal/mvcc/mvcc.go`의 `Begin`
- 처음 가설:
  `Begin` 하나를 이해하면 나머지 흐름도 거의 자동으로 따라올 거라고 생각했다.
- 실제 진행:
  `rg -n "Begin|Read" internal cmd`로 핵심 함수 위치를 다시 잡고, `Begin`가 문제 정의의 첫 번째 bullet과 정확히 맞물리는지 확인했다.
...
`Begin`는 이 프로젝트의 write path 혹은 primary decision point를 드러낸다. 테스트가 요구하는 첫 번째 조건을 만족시키는 규칙이 여기서 한 번에 보였다.
...
- 당시 목표:
  `Read`가 `Begin`와 어떤 짝을 이루는지 확인한다.
- 변경 단위:
  `database-systems/go/database-internals/projects/08-mvcc/internal/mvcc/mvcc.go`의 `Read`
- 처음 가설:
  `Read`는 단순 보조 함수일 거라고 생각했다.
- 실제 진행:
  두 번째 앵커를 읽고 나니, 실제로는 `Begin`가 만든 상태를 외부에서 관찰 가능하게 만드는 규칙이 여기 있었다.

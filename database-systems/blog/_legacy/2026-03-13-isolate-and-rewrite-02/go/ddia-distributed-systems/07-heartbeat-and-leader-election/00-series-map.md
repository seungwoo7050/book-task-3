# 07 Heartbeat and Leader Election — Series Map

heartbeat 기반 failure detector와 majority vote만으로 leader failover를 재현하는 작은 election lab을 구현합니다. 이 시리즈는 기존 초안의 말투를 따라가지 않고, 실제 코드와 검증 신호를 다시 읽으면서 판단이 어디서 바뀌는지에만 집중한다.

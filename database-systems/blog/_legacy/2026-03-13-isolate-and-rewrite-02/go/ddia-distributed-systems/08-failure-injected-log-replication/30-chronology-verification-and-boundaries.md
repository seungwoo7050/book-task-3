# 30 08 Failure-Injected Log Replication를 다시 검증하고 경계를 닫기
...
`TestPausedFollowerLagsButRecoversAfterResume`는 구현의 뒷부분에서 생길 수 있는 붕괴 지점을 문장보다 정확하게 고정한다. pass 숫자보다 중요한 건, 어떤 경계가 계속 회귀 대상으로 남아 있느냐였다.
...
- 실제 진행:
  `GOWORK=off go run ./cmd/failure-replication`를 다시 실행해 마지막 한 줄을 확인하고, README의 `한계와 확장` bullet과 나란히 읽었다.

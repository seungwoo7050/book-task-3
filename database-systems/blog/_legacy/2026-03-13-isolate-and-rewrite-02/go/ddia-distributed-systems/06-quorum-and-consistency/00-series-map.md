# 06 Quorum and Consistency — Series Map

quorum read/write와 versioned register를 이용해 `W + R > N`이 최신 읽기를 어떻게 보장하고, `W + R <= N`일 때 어떤 stale read가 생기는지 재현합니다. 이 시리즈는 기존 초안의 말투를 따라가지 않고, 실제 코드와 검증 신호를 다시 읽으면서 판단이 어디서 바뀌는지에만 집중한다.

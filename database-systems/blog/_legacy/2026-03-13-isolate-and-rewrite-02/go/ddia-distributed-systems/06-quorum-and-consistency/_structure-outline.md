# 06 Quorum and Consistency — Structure Outline
...
- `20-chronology-core-invariants.md`: `LatestVersion`와 `ReadResult`가 실제로 invariant를 고정하는 구간
...
1. `10-chronology-scope-and-surface.md`
   ...
   코드 앵커: `TestReadReturnsLatestWhenQuorumsOverlap`, `LatestVersion`
   CLI: `find internal tests cmd -type f | sort`, `rg -n "^func Test" tests`

2. `20-chronology-core-invariants.md`
   핵심 invariant가 `LatestVersion`와 `ReadResult` 사이에서 어떻게 고정되는지 보여 준다.
   코드 앵커: `LatestVersion`, `ReadResult`
   CLI: `rg -n "^(type|func) " internal cmd`, `rg -n "LatestVersion|ReadResult" internal cmd`

3. `30-chronology-verification-and-boundaries.md`
   테스트와 demo를 모두 남겨, pass 신호와 공개 표면을 구분해 설명한다.
   코드 앵커: `TestReplicaFailuresReduceAvailability`, `main.go`
   CLI: GOWORK=off go test ./...; GOWORK=off go run ./cmd/quorum-demo

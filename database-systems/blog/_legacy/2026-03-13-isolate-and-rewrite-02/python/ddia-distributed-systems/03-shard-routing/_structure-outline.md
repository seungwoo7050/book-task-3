...
- `20-chronology-core-invariants.md`: `Router`와 `hash_value`가 실제로 invariant를 고정하는 구간
...
1. `10-chronology-scope-and-surface.md`
   ...
   코드 앵커: `test_empty_and_single_node_routing`, `Router`
   CLI: `find src tests -type f | sort`, `rg -n "^def test_" tests`

2. `20-chronology-core-invariants.md`
   핵심 invariant가 `Router`와 `hash_value` 사이에서 어떻게 고정되는지 보여 준다.
   코드 앵커: `Router`, `hash_value`
   CLI: `rg -n "^(class|def) " src`, `rg -n "Router|hash_value" src`

3. `30-chronology-verification-and-boundaries.md`
   테스트와 demo를 모두 남겨, pass 신호와 공개 표면을 구분해 설명한다.
   코드 앵커: `test_batch_routing`, `__main__.py`
   CLI: PYTHONPATH=src .venv/bin/python -m pytest; PYTHONPATH=src .venv/bin/python -m shard_routing

...
```bash
mkdir -p go/database-internals/projects/01-memtable-skiplist/{cmd/skiplist-demo,internal/skiplist,tests,problem/{code,data,script},docs/{concepts,references}}
```
...
go mod init study.local/go/database-internals/projects/01-memtable-skiplist
```
...
```bash
# workspace 루트에서
go work use go/database-internals/projects/01-memtable-skiplist
```

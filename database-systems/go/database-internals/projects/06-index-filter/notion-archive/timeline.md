...
```bash
mkdir -p go/database-internals/projects/06-index-filter/{cmd/index-filter,internal/{bloomfilter,sparseindex,sstable},tests,problem/{code,data,script},docs/{concepts,references}}
```
...
go mod init study.local/go/database-internals/projects/06-index-filter
```
...
```bash
go work use go/database-internals/projects/06-index-filter
```

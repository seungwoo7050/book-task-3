...
```bash
mkdir -p go/database-internals/projects/05-leveled-compaction/{cmd/leveled-compaction,internal/{compaction,sstable},tests,problem/{code,data,script},docs/{concepts,references}}
```
...
go mod init study.local/go/database-internals/projects/05-leveled-compaction
```
...
```bash
go work use go/database-internals/projects/05-leveled-compaction
```

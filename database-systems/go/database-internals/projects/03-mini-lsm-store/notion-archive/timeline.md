...
```bash
mkdir -p go/database-internals/projects/03-mini-lsm-store/{cmd/mini-lsm-store,internal/{lsmstore,skiplist,sstable},tests,problem/{code,data,script},docs/{concepts,references}}
```

### Go 모듈 초기화

```bash
cd go/database-internals/projects/03-mini-lsm-store
go mod init study.local/go/database-internals/projects/03-mini-lsm-store
```
...
```bash
# workspace 루트에서
go work use go/database-internals/projects/03-mini-lsm-store
```
...
```bash
cp go/database-internals/projects/01-memtable-skiplist/internal/skiplist/skiplist.go \
...
```bash
cp go/database-internals/projects/02-sstable-format/internal/sstable/sstable.go \

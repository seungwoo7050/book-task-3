...
```bash
mkdir -p go/database-internals/projects/04-wal-recovery/{cmd/wal-recovery,internal/{wal,store,skiplist,sstable},tests,problem/{code,data,script},docs/{concepts,references}}
```
...
go mod init study.local/go/database-internals/projects/04-wal-recovery
```
...
```bash
go work use go/database-internals/projects/04-wal-recovery
```

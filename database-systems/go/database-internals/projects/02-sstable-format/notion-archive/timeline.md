...
```bash
mkdir -p go/database-internals/projects/02-sstable-format/{cmd/sstable-format,internal/sstable,tests,problem/{code,data,script},docs/{concepts,references}}
```
...
go mod init study.local/go/database-internals/projects/02-sstable-format
```
...
```bash
cd go/shared
go mod init study.local/go/shared
```
...
```bash
# workspace 루트에서
go work use go/database-internals/projects/02-sstable-format
```

...
3. `internal/`와 `tests/`를 함께 읽고, 마지막에 `cmd/wal-recovery/`로 동작 예시를 확인합니다.
...
- `internal/skiplist/`, `internal/sstable/`, `internal/store/`, `internal/wal/`, `tests/`, `cmd/wal-recovery/`: 실제 구현과 검증 코드
- `notion/`: 현재 공개용 학습 노트

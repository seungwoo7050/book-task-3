...
이 문서는 05 Clustered KV Capstone 프로젝트에서 “무엇을 구현해야 하는가”를 현재 기준으로 다시 설명합니다. 과거 과제군에서 출발한 아이디어는 남기되, 현재 레포에 없는 로컬 경로를 전제로 설명하지는 않습니다.
...
- 현재 프로젝트에서의 재구성: Go 트랙에서는 전체 분산 커리큘럼의 브리지 프로젝트로 두어, 저장 엔진과 클러스터 개념이 실제 요청 흐름에서 만나는 지점을 보여 줍니다.

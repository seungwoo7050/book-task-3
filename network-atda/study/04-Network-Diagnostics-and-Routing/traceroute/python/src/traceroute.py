...
UDP probe와 ICMP reply를 이용해 target host까지의 경로를 추적한다.
...
def parse_icmp_response(packet: bytes) -> tuple[int, int, int | None] | None:
    """raw ICMP reply에서 `(icmp_type, icmp_code, embedded_udp_dest_port)`를 꺼낸다."""
    if len(packet) < 20:
        return None
...
def format_hop_line(ttl: int, observations: list[ProbeObservation]) -> str:
    """terminal 출력용 traceroute hop 문자열을 만든다."""
    probe_parts = []
    responders: list[str] = []

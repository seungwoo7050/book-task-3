...
## 먼저 읽을 문서
- [05-development-timeline.md](05-development-timeline.md): 처음부터 `verified`까지 따라가는 재현 가이드. parser test와 live trace를 분리해 재현하는 데 가장 중요하다.
...
## 이 프로젝트에서 특히 남긴 주제
- TTL 기반 hop 발견 원리
- embedded UDP port로 probe를 매칭하는 이유
- 도착지 판정과 출력 포맷
- parser test와 live traceroute를 분리해 재현하는 순서

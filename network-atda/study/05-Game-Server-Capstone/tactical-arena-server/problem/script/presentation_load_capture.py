...
def running_server(build_dir: Path):
    tcp_port = free_port()
    udp_port = free_port()
    temp_dir = Path(tempfile.mkdtemp(prefix="arena-load-presentation-"))
...
def main() -> int:
    ...
    with running_server(build_dir) as (tcp_port, db_path):
        ...
        transcript.append(completed.stdout.strip())
        ...
        try:
            ...
            cur.execute("SELECT winner_player_id, result_blob FROM match_history ORDER BY id")
    ...
    for index, row in enumerate(history, start=1):
        transcript.append(f"match_{index} winner={row[0]} result={row[1]}")

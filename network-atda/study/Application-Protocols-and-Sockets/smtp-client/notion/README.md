...
## 먼저 읽을 문서
- [05-development-timeline.md](05-development-timeline.md): 처음부터 `verified`까지 따라가는 재현 가이드. SMTP 대화를 순서대로 다시 보는 데 가장 중요하다.
...
## 이 프로젝트에서 특히 남긴 주제
- `220`/`250`/`354`/`221` 응답 코드의 의미
- SMTP envelope와 메일 헤더를 구분하는 이유
- `CRLF`와 `DATA` 종료 규칙
- 로컬 mock 서버 기준으로 끝까지 재현하는 순서

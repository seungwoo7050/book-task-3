...
```bash
cd study/Application-Protocols-and-Sockets/udp-pinger/python/tests
...
```bash
make -C study/Application-Protocols-and-Sockets/udp-pinger/problem run-server
```

터미널 2:

```bash
make -C study/Application-Protocols-and-Sockets/udp-pinger/problem run-solution
```

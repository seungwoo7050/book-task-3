...
이 문서는 DNS Packet Analysis를 시작하기 전에 읽는 현재 저장소 기준 문제 사양입니다. 답안을 먼저 보기보다 trace 범위와 질문을 먼저 이해하는 데 초점을 둡니다.
...
- `../study/03-Packet-Analysis-Top-Down/dns/problem/data/dns-nslookup.pcapng`부터 읽으면 가장 짧게 구현을 시작할 수 있다.
...
- `../study/03-Packet-Analysis-Top-Down/dns/problem/data/dns-nslookup.pcapng` 등 fixture나 trace를 읽지 않고 동작을 추측해서 구현하지 않는다.
- 상위 카탈로그 요약만 보고 세부 산출물 계약을 생략하지 않는다.
- 검증 명령이 통과한다고 해서 입력 계약과 경계 조건까지 자동으로 맞는다고 가정하지 않는다.

## 성공 체크리스트

- `../study/03-Packet-Analysis-Top-Down/dns/problem/data/dns-nslookup.pcapng` 등 fixture/trace 기준으로 결과를 대조했다.
- `make -C /Users/woopinbell/work/book-task-3/network-atda/study/03-Packet-Analysis-Top-Down/dns/problem test`가 통과한다.
...
- `dns`의 Makefile이 호출하는 하위 toolchain이 현재 셸에서 동작해야 한다.

## 스포일러 경계

정답 코드, 공식 구현 진입점, 해설은 [`dns_answer.md`](dns_answer.md)에서 확인한다.

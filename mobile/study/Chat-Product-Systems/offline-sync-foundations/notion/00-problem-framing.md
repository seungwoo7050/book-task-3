...
이 프로젝트는 `Chat-Product-Systems` 그룹의 첫 번째 과제다. 여기서 다진 queue/retry/idempotency 패턴이 이후 `realtime-chat`의 pending message 모델과 `incident-ops-mobile-client`의 persistent outbox에 그대로 재사용된다.

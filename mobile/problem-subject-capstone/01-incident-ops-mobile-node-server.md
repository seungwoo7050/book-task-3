# 01-incident-ops-mobile-node-server 문제지
...
- `../study/capstone/01-incident-ops-mobile/node-server/package-lock.json` 등 fixture나 trace를 읽지 않고 동작을 추측해서 구현하지 않는다.
- 같은 주제의 다른 runtime 구현을 섞어 읽지 않는다.

## 성공 체크리스트

- `../study/capstone/01-incident-ops-mobile/problem/code/contracts/contracts.ts`의 빈 확장 지점을 실제 구현으로 채웠다.
- 핵심 흐름은 `OfflineQueueEngine`와 `assertOk`가 각각 어떤 상태 전이와 계산을 맡는지 코드로 설명할 수 있다.
- 검증 기준은 `server`와 `login`가 잠근 입력 계약과 회귀 경계를 그대로 재현하는 것이다.
- `../study/capstone/01-incident-ops-mobile/node-server/package-lock.json` 등 fixture/trace 기준으로 결과를 대조했다.
...
정답 코드, 공식 구현 진입점, 해설은 [`01-incident-ops-mobile-node-server_answer.md`](01-incident-ops-mobile-node-server_answer.md)에서 확인한다.

...
int main() {
    ...
    TEST("negate(TMin)", negate(INT_MIN) == INT_MIN);
    ...
    TEST("floatFloat2Int(+inf)",  floatFloat2Int(0x7F800000u) == static_cast<int>(0x80000000u));
    TEST("floatFloat2Int(NaN)",   floatFloat2Int(0x7FC00000u) == static_cast<int>(0x80000000u));

    TEST("floatPower2(0)",        floatPower2(0) == 0x3F800000u);
    TEST("floatPower2(1)",        floatPower2(1) == 0x40000000u);
    TEST("floatPower2(128)",      floatPower2(128) == 0x7F800000u);
    TEST("floatPower2(-150)",     floatPower2(-150) == 0u);

    std::printf("\n=== %d / %d C++ edge-case tests passed ===\n", passes, passes + fails);
    return fails ? 1 : 0;
}

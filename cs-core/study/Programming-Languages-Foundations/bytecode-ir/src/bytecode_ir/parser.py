...
class Parser:
    ...
    def _parse_expression(self, min_precedence: int = 0) -> Expr:
        ...
        while True:
            ...
            left = BinaryOp(BINARY_OPERATOR[operator.kind], left, right, operator.line, operator.column)
        return left
    ...
    def _parse_prefix(self) -> Expr:
        ...
        if token.kind in UNARY_OPERATOR:
            ...
            return UnaryOp(UNARY_OPERATOR[operator.kind], operand, operator.line, operator.column)
    ...
    def _parse_primary(self) -> Expr:
        ...
        if token.kind == "INTEGER":
            self._advance()
            return IntegerLiteral(int(token.lexeme), token.line, token.column)
        if token.kind == "TRUE":
            self._advance()
    ...
    def _parse_postfix(self, expression: Expr) -> Expr:
        while self._match("LPAREN"):
            open_token = self.tokens[self.index - 1]
            arguments: list[Expr] = []
            ...
            expression = CallExpr(expression, tuple(arguments), open_token.line, open_token.column)
        return expression

    def _parse_let(self) -> Expr:
        let_token = self._expect("LET", "expected 'let'")
        ...
        return LetExpr(name.lexeme, annotation, value, body, let_token.line, let_token.column)

    def _parse_if(self) -> Expr:
        if_token = self._expect("IF", "expected 'if'")
        ...
        return IfExpr(condition, then_branch, else_branch, if_token.line, if_token.column)

    def _parse_fun(self) -> Expr:
        fun_token = self._expect("FUN", "expected 'fun'")
        ...
        if not self._check("RPAREN"):
            while True:
                ...
                if self._match("COLON"):
                    annotation = self._parse_type()
                params.append(Param(param_name.lexeme, annotation, param_name.line, param_name.column))
                if not self._match("COMMA"):
                    break
        ...
        body = self._parse_expression()
        return FunExpr(tuple(params), return_annotation, body, fun_token.line, fun_token.column)
    ...
    def _parse_type(self) -> TypeExpr:
        ...
        if grouped is not None:
            raise self._error("multi-parameter type list must be followed by '->'", self._current())
        return head

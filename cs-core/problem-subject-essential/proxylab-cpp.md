...
- 핵심 흐름은 `remove_entry`와 `insert_front`가 각각 어떤 상태 전이와 계산을 맡는지 코드로 설명할 수 있다.
- `../study/Systems-Programming/proxylab/problem/script/driver.sh` fixture/trace 기준으로 결과를 대조했다.
- `make -C /Users/woopinbell/work/book-task-3/cs-core/study/Systems-Programming/proxylab/cpp test`가 통과한다.
...
정답 코드, 공식 구현 진입점, 해설은 [`proxylab-cpp_answer.md`](proxylab-cpp_answer.md)에서 확인한다.

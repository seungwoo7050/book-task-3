...
이 트랙은 채용용으로 설명 가능한 제품형 결과물을 만드는 단계다. 내부도구형 UI와 고객-facing UI를 각각 한 프로젝트씩 두어, 서로 다른 프론트 문제를 어떻게 푸는지 보여 준다.

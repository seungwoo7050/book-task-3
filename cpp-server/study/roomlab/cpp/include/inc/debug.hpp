...
#define STRINGIFY(x) ""#x	// handle empty string
...
namespace debug
{
	...
	void	logl(const std::string &msg, const std::string &color);	/* flush msg with a newline */
	...
	// void	log_msg_status(const int comm, const std::string &prefix, const std::string &command, const std::vector<std::string> &params);

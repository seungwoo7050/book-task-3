...
[`tests/test_arenaserv.py`](../../../game-track/02-arenaserv/cpp/tests/test_arenaserv.py)는 이 capstone proof를 세 시나리오로 나눠 보여 준다. `scenario_duel_and_rejoin()`은 duplicate nick 거절, grace 안 rejoin 성공, grace 밖 rejoin 실패, invalid input, stale sequence, 세 번의 `HIT` 뒤 `ELIM`과 `ROUND_END alpha`를 본다. `scenario_party_lobby()`는 3인과 4인 lobby, `room_full`, countdown, `SNAPSHOT 0`를 확인한다. `scenario_draw_timeout()`는 입력이 없어도 timed tick이 계속 흘러 `ROUND_END draw`가 도착하는지 본다.

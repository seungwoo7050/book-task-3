...
def test_demo_cli_writes_demo_assets_and_report_sections(tmp_path: Path) -> None:
    ...
    assert result.exit_code == 0
    assert result.stdout.strip() == f"demo assets written to {tmp_path}"
    report = (tmp_path / "07-report.md").read_text()
    assert "# Service Summary" in report
    assert "# Crypto Findings" in report
    assert "# Auth Findings" in report
    assert "# Backend Findings" in report
    assert "# Dependency Queue" in report
    assert "# Remediation Board" in report
